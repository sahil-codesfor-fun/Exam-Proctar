...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Dept. Heads', icon: UserCog, path: '/superadmin/department-heads' },
    { name: 'Teachers', icon: GraduationCap, path: '/superadmin/teachers' },
    { name: 'Students', icon: Users, path: '/superadmin/students' },
    { name: 'Subjects', icon: BookOpen, path: '/superadmin/subjects' },
    { name: 'Exams', icon: FileText, path: '/superadmin/exams' },
    { name: 'Reports', icon: BarChart3, path: '/superadmin/reports' },
    { name: 'Activity Logs', icon: Activity, path: '/superadmin/activity-logs' },
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 text-gray-900 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-b border-gray-100">
          <p className="text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-1">Super Admin</p>
