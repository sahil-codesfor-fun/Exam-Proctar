...
const AllocateCoursesModal = ({ department, onClose, onRefresh }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.normalizedMessage || 'Failed to load courses. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAllocate = async () => {
    ...
    try {
      ...
      setError(err.normalizedMessage || 'Failed to allocate courses.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      ...
      setError(err.normalizedMessage || 'Failed to create course.');
    } finally {
  ...
  const handleRemove = async (courseId) => {
    ...
    try {
      ...
      setError(err.normalizedMessage || 'Failed to remove course.');
    }
  };
  ...
  const confirmDeleteCourse = async () => {
    ...
    try {
      ...
      await fetchData();
      if (onRefresh) await fetchData();
    } catch (err) {
      setError(err.normalizedMessage || 'Failed to delete course globally.');
    } finally {
      setSaving(false);
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex-1 overflow-y-auto min-h-[300px]">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-gray-500">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">No courses have been created yet.</h3>
                  <p className="text-sm text-gray-400 mb-4 max-w-sm text-center">
                    Get started by creating your first course to allocate it to {department.name}.
                  </p>
                  <button 
                    onClick={() => setShowCreateCourse(true)}
