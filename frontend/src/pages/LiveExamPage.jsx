...
export const LiveExamPage = () => {
  ...
  const [phase, setPhase] = useState('loading'); // loading | fullscreen | exam | submitted | error | restricted
  ...
  const proctoring = useProctoring({
    examId, enabled: phase === 'exam',
    maxViolations: exam?.proctoring?.maxViolations || 3,
    onRestricted: handleRestricted, onAutoSubmit: handleAutoSubmit,
  });

  // AI Vision
  const aiVision = useAIVision({
    enabled: phase === 'exam' || phase === 'camera_check',
  ...
  useEffect(() => {
    ...
    // Prevent tab close during exam
    const handleBeforeUnload = (e) => {
      if (phase === 'exam' || phase === 'fullscreen') {
        e.preventDefault();
        e.returnValue = '';
      }
    };
    ...
    // Enforce Fullscreen Auto-Return
    const handleFullscreenChange = () => {
      if (phase === 'exam' && exam?.proctoring?.requireFullscreen && !document.fullscreenElement) {
        setPhase('fullscreen');
      }
    };
  ...
  if (phase === 'error' || phase === 'restricted') return (
    <div className="h-screen flex items-center justify-center bg-gray-950 text-white">
    ...
    <div className="h-screen flex items-center justify-center bg-gray-950 text-white p-4">
      <div className="text-center max-w-2xl w-full p-8 bg-gray-900 rounded-3xl border border-gray-800 shadow-2xl">
        ...
        <div className="bg-black rounded-2xl overflow-hidden mb-8 mx-auto w-full max-w-md h-72 border border-gray-700 relative shadow-inner">
          <video ref={aiVision.videoRef} className="w-full h-full object-cover" style={{ transform: 'scaleX(-1)' }} playsInline muted autoPlay />
  ...
  // ── EXAM PHASE ──
  if (phase === 'exam' && (!exam || !answers.length)) return (
    <div className="h-screen flex items-center justify-center bg-gray-950 text-white">
  ...
  if (phase === 'exam' && !q) return (
    <div className="h-screen flex items-center justify-center bg-gray-950 text-white">
      <div className="text-center"><div className="text-4xl mb-4 animate-spin">⌛</div><p className="text-gray-400">Loading question data…</p></div>
    </div>
  );
    ...
    <div className="h-screen flex flex-col bg-gray-950 text-white overflow-hidden select-none" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-gray-900 border-b border-gray-800 flex-shrink-0">
        <div className="flex items-center gap-4">
          <span className="text-lg font-bold">🛡️ {exam?.title || 'Loading Exam...'}</span>
        ...
        <button onClick={() => doSubmit(false)} disabled={submitting}
          className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold py-2 px-6 rounded-lg text-sm">
