...
const SuperAdminSubjects = () => {
  ...
  const fetchSubjects = async () => {
    try {
      setLoading(true);
      const res = await api.get('/superadmin/subjects');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/superadmin/subjects/${editingId}`, formData);
      } else {
        await api.post(`/superadmin/subjects`, formData);
      }
      setShowModal(false);
      setEditingId(null);
      setFormData({ name: '', code: '', semester: '', credits: '', departmentId: '', status: 'ACTIVE' });
      fetchSubjects();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Subject Master List</h2>
          <p className="text-gray-500 mt-1">Manage subjects across all departments.</p>
        </div>
        <button 
          onClick={() => {
            setEditingId(null);
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 bg-gray-50 rounded-t-xl">
        ...
        <div className="p-0 overflow-x-auto">
          <table className="w-full text-left min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-gray-500">Loading subjects...</td>
                </tr>
              ) : filteredSubjects.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-gray-500">No subjects found.</td>
                </tr>
              ) : (
