...
const AdminDashboard = () => {
  ...
  const departments = ['All']; // Could be dynamic if needed
  ...
  const fetchFaculty = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/admin/teachers?search=${search}&department=${deptFilter}`);
  ...
  const handleCreateFaculty = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.post('/admin/teachers', newFaculty);
      if (res.data.success) {
        setGeneratedCreds({
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      await api.put(`/admin/teachers/${id}`, { status: newStatus, isActive: newStatus === 'ACTIVE' });
      fetchFaculty();
  ...
  const handleHardDelete = (id) => {
    showConfirm("Are you sure you want to PERMANENTLY DELETE this faculty account? This action cannot be undone.", async () => {
      try {
        await api.delete(`/admin/teachers/${id}/hard`);
        fetchFaculty();
  ...
  const saveSubjects = async () => {
    try {
      await api.post(`/admin/teachers/${subjectModal.teacherId}/subjects`, {
  ...
  const openDrawer = async (teacher) => {
    ...
    try {
      const [loginRes, actRes] = await Promise.all([
        api.get(`/admin/teachers/${teacher.id}/login-history`),
        api.get(`/admin/teachers/${teacher.id}/activity`)
      ]);
