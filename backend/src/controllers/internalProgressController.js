...
export const getUnifiedDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      prisma.studentCodingProgress.findUnique({
        where: { studentId },
        select: {
          totalAssigned: true,
          totalSolved: true,
      ...
      prisma.studentCodingStatistics.findUnique({
        where: { studentId },
        select: {
          currentStreak: true,
          longestStreak: true,
          ...
          totalAttempts: true,
          wrongAttempts: true,
          solvedOnFirstAttempt: true
        }
      }),
      prisma.topicProgress.findMany({
        where: { studentId },
        select: { topic: true, solvedCount: true, totalAvailable: true }
      }),
    ...
    const solvedQuestions = questionIds.length > 0
