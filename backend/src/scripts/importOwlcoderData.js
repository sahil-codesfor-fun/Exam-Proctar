 ...
 * 
 * Usage: node src/scripts/importOwlcoderData.js
 */
...
const DATA_DIR = 'c:\\D\\D\\nnsbbsh\\Downloads\\work\\Higly confidential';
...
async function importQuestions(courseTitle, filePath, hasCategory, moduleMap) {
  ...
  for (const row of rows) {
    const owlcoderId = sanitize(row._id || row['_id']);
    ...
    // Skip duplicates
    if (owlcoderId) {
      const existing = await prisma.question.findUnique({ where: { owlcoderId } });
      if (existing) {
        skipped++;
        continue;
      }
    ...
    try {
      ...
      if (owlcoderId) {
        const q = await prisma.question.findUnique({ where: { owlcoderId } });
        if (q) {
          // Visible test cases
...
async function importArticles(filePath, moduleId) {
  ...
  for (const row of rows) {
    ...
    // Skip duplicates
    if (owlcoderId) {
      const existing = await prisma.hubArticle.findUnique({ where: { owlcoderId } });
      if (existing) {
        skipped++;
        continue;
      }
...
// ── Main ────────────────────────────────────────────────────────
async function main() {
  console.log('🚀 Starting Owlcoder Data Import...\n');
    ...
    if (!course) {
      course = await prisma.hubCourse.create({
        data: {
          ...
          description: `${courseDef.title} course imported from Owlcoder platform.`,
        },
      });
